...
```
  +--------+                                           +---------------+
  |        |--(C)--------- Login Credentials --------->| Authorization |
  |        |                                           |     Server    |
  |        |<-(D)----------- Access Token -------------|               |
  |        |                                           +---------------+
  | Client |
...
authorization. In addition to that OAuth2 establishes scopes which are
different permission-levels (e.g. read-only vs. read-and-write).
...
OAuth2 proposes the mechanism mentioned above (the pretty charts) but does
not postulate any specific implemention of the encryption along with a
payload.

JWT's actually implement the symmetric or asymmetric encryption of that
payload.
