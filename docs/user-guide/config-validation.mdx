...
1. Checking whether the survey settings are valid _(see below)_
2. Checking whether all field-configs are valid _(see below)_
3. If `(config.authentication == 'email')` then there has to be exactly one email field in config
