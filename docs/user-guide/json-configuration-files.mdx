...
- `end [integer]`: `start <= end`
- `draft [boolean]`
- `authentication [string]`: one of `[open, email, invitation]`
- `limit[integer]`: `0 <= limit`, when no payment info is provided, surveys are automatically capped at 100 submissions
- `fields [list]`: `1 <= fields.length <= 100`, if `authentication == email` this must include exactly one email field, otherwise no email field at all
